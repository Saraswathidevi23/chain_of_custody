...
def add_evidence():
    ...
    if request.method=='POST':
        ...
        dd = mycursor.fetchone()
        dtime=str(dd[4])
        bdata="Evidence ID:"+str(maxid)+", Case ID:"+case_id+", Status: Evidence File: "+efile+", Upload by admin, Date: "+dtime
        ###
...
def access():
    ...
    return render_template('access.html',msg=msg,data=data,act=act,cid=cid,case_id=case_id,data2=data2,bc=bc,bdata=bdata,st=st,eid=eid)
...
def verify():
    ...
    return render_template('verify.html',msg=msg,data=data,mess=mess,act=act,uname=uname,key=key)
